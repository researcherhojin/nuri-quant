...
if [ "$SCHEMA_VERSION" -ge 40 ]; then
    echo " ✅ schema version: $SCHEMA_VERSION (>=40)"
else
    echo " ❌ schema version: $SCHEMA_VERSION (need >=40 for #529 Phase 1+2)"
    EXIT_CODE=2
fi

# 2) Required Phase 1+2 tables
for table in agent_audit_ledger feature_flags agent_run_ledger agent_messages walkforward_runs regime_posteriors hypotheses causal_audits agent_decisions decision_outcomes execution_blocks incidents dr_replicas collector_runs drift_alerts foundation_benchmarks; do
