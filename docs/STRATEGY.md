...
| 벤치마크 | SPY (`forward_outcome_tracker.py` `DEFAULT_BENCHMARK_TICKER` = `measurement_mode.benchmark`). **본 판정은 US-only 로 고정.** #833 착륙 후 KR 결정의 **기록 기준**은 기초자산별 (`measurement_mode.benchmark_by_asset_class`, 분류 = `siege_gates.asset_class_rules`, 매 outcome 행의 `benchmark_ticker` 에 자기기술; KRX 상장 US 추종 ETF → SPY, 채권·원자재 → 벤치마크 없음 = alpha NULL, 그 외 KR → KOSPI. #833 은 상장 시장 기준이었고 #1459 가 기초자산 기준으로 고쳤다) — 기록 기준이 바뀌었을 뿐 **판정 대상 여부는 그대로**이며, KR 은 **별도 사전등록** 전까지 진단 전용 | #675 caveat: SPY 는 growth 대비 과대평가. KR 을 SPY 로 재면 FX + 시장 스타일이 alpha 에 섞여 부호까지 뒤집힘 |
...
**미구축 (판정 전 선결, follow-up issue)**: ① regime 라벨 백필 — #832 구현 완료 (`scripts/ops/backfill_regime_labels.py` + emit 경로 canonical-or-NULL, 진단용) ② 순열 판정 도구 — #842 구현 완료 (`nuri/quant/validation/decision_alpha.py`, 설계는 본 표에 사전 고정; 기존 `nuri/quant/validation/` 3종은 포트폴리오 Sharpe 전용) ③ 3조건 통합 판정 쿼리 (`/api/alpha` 는 착륙 전 NOT_MEASURABLE 유지) ④ KR benchmark 분리 — #833 구현 완료 (`decision_outcomes.benchmark_ticker`, 기록 기준만; KR 판정 사전등록은 미착수). #1459 (2026-09-08): 기록 기준을 상장 시장이 아니라 **기초자산**으로 — `measurement_mode.benchmark_by_asset_class` (분류 = `siege_gates.asset_class_rules`). KRX 상장 US 추종 ETF → SPY, 채권·원자재 → 벤치마크 없음(alpha NULL). 원장 실측 KOSPI 행 1,200 중 525 가 비-한국 기초자산이었고 US 지수 ETF 의 30d "alpha" −15%p 는 KOSPI−S&P 스프레드였다. **US 상장 결정(판정 표본)은 SPY 고정 그대로** — 이 map 을 보지 않는다. **쌓인 행은 다음 일일 스캔이 새 규칙으로 재측정한다** — `scheduler.py` `alpha_tracking` (17:00 KST) 이 `max_decisions=2000` 으로 도는데 emit 된 결정이 1,247 건이라 전부 걸리고, scan 쿼리에 '이미 측정됨' 필터가 없으며 upsert 가 `ON CONFLICT … DO UPDATE SET benchmark_ticker = excluded.benchmark_ticker, alpha = excluded.alpha` 다. 어느 규칙이 만든 행인지는 같은 upsert 가 갱신하는 `decision_outcomes.code_rev` · `tracked_as_of_date` (#1305) 로 가린다. 판정 표본은 불변 — `decision_alpha.py` 의 `is_kr_ticker → continue` 가 alpha 수집과 `n_missing_closed` 계상 **양쪽보다 앞**이라 새 NULL 행이 15% 무효화 지표를 못 움직인다 ⑤ 슬리브 상한 소비 배선 (rebalance_advisor / ExecutionFirewall, #834) ⑥ 원장 스냅샷/백업 정책 (#835). 월간 알파 진행 리포트 표출 = #856.
...
### 4.1 테스트
| 항목 | 기준 | 현재 |
| Backend tests | Codecov 1% relative regression (목표 ≥ 95%) | 8,334 tests, 383 files (statement coverage **99%** — 17/23,311 미커버 9개 파일, partial branch 81, `make ci-cov` 2026-08-14) |
