...
|------|---|------|------|---------|------|------------|
| 🔴 P0 | 0 | **E3 Symmetric Amplifier — Phase 2 paired counterfactual** | — | feat(engine) | 1-2 세션 | Phase 1 shadow shipped (#479, 2026-04-29 — `amplifier_gate.py` + `recovery_detector.py` + 26 lock-tests, all `enabled: false`). 다음: Stage 2 paired counterfactual on 20d breakout entries (N≥200), STRATEGY §3.6 acceptance. Source plan `docs/plans/E3_symmetric_amplifier_design.md` Phase 2 spec. PASS verdict 후 Phase 3 (alpha-amplified live, conf boost only). Kill-switch invariant 유지: trailing paired excess < 0 + CI_lower < 0 (n≥10) → auto 강등. |
