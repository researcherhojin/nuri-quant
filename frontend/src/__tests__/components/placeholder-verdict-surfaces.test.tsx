...
describe("근거 사슬 — 자리표시자를 근거로 세지 않는다 (#1436, codex R15)", () => {
  ...
  it("축이 없는 과거 evidence 행은 둘 다 false — 없던 분류를 지어내지 않는다", () => {
    ...
    // 파싱은 되는데 객체가 아닌 경우 — 배열·스칼라·null. `not json` 은 catch 로 빠져서
    // 이 분기를 안 밟는다(파싱 실패와 "파싱은 됐는데 모양이 다름" 은 다른 경로다).
    expect(parseDetailFlags("[1,2]")).toEqual({ degraded: false, abstained: false });
    expect(parseDetailFlags("42")).toEqual({ degraded: false, abstained: false });
    expect(parseDetailFlags("null")).toEqual({ degraded: false, abstained: false });
  });
});
