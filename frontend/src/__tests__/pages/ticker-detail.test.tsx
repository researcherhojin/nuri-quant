...
describe("TickerPage — 자리표시자는 의견이 아니다 (#1436)", () => {
    ...
    const withPlaceholder = {
      ...
      consensus: {
        ...
        verdicts: [
          ...
          // 조건식의 반대쪽 — degraded 라벨도 밟는다
          { agent_name: "crypto", action: "HOLD", confidence: 0, degraded: true },
        ],
      },
    };
    mockFetchAPI.mockImplementation((url: string) => {
  ...
  it("기권 에이전트의 확신도를 숫자로 찍지 않는다", async () => {
    ...
    const cry = screen.getByText("crypto").closest("div")!.parentElement!;
    expect(within(cry).getByText(CONSENSUS.PLACEHOLDER_DEGRADED)).toBeInTheDocument();
