...
async function EvidenceCharts() {
  ...
  return (
    <div className="space-y-4">
      <ChartCard title={E.TITLE_REGIME} empty={!regime || regime.count === 0}>
        {regime && <RegimeChartLazy data={regime} />}
      </ChartCard>

      <ChartCard title={E.TITLE_HEATMAP} empty={!heatmap || heatmap.count === 0}>
        {heatmap && <PortfolioTreemapLazy data={heatmap} />}
      </ChartCard>

      <ChartCard title={E.TITLE_SIGNALS} empty={!signals || signals.count === 0}>
        {signals && <SignalPerformanceChartLazy data={signals} />}
      </ChartCard>

      <ChartCard title={E.TITLE_FEAR_GREED} empty={!fearGreed || fearGreed.count === 0}>
