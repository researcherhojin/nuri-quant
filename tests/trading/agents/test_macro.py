...
class TestMacroAgentBranches:
    ...
    def _mock_regime(self, monkeypatch, trend, regime_name, macro_score):
        ...
        regime_obj = FakeRegime(regime=regime_name, trend=trend)
        macro_obj = FakeMacro(total_score=macro_score)

        import nuri.quant.regime.classifier as cls_mod
        import nuri.quant.regime.macro_score as ms_mod

        monkeypatch.setattr(cls_mod, "classify_regime", lambda db_path=None: regime_obj)
        monkeypatch.setattr(ms_mod, "compute_macro_score", lambda db_path=None: macro_obj)
