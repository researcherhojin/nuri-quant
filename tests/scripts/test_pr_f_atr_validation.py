...
_SCRIPT_PATH = Path(__file__).resolve().parents[2] / "scripts" / "episodes" / "pr_f_atr_validation.py"
...
class TestGridFrozen:
    ...
    def test_k_grid_parity_with_atr_module(self):
        ...
        assert v.K_GRID == K_GRID, (
            "validation script K_GRID drifted from atr module — paired counterfactual 재생산 불가"
        )
...
class TestPairedDeltas:
    ...
    def _trade(
        self, ticker: str, baseline_ret: float | None, treat_ret: float | None, k: float = 2.0, mult: float = 1.0
    ) -> "v.Trade":  # type: ignore[name-defined]
        return v.Trade(
            ticker=ticker,
