...
class TechnicalAgent(BaseAgent):
    ...
    def analyze(self, ticker: str, db_path=None) -> AgentVerdict:
        ...
        if (
            df.empty or len(df) < min_dp
        ) and db_path is None:  # pragma: no cover — yfinance fallback (network, prod-only)
            try:
        ...
        if not df.empty and "date" in df.columns:
            ...
            except Exception:  # pragma: no cover — chart pattern analyzer best-effort
                chart = None
    ...
    def _get_finviz_signals(self, ticker: str, db_path=None) -> list[str]:
        ...
        except Exception:  # pragma: no cover — FINVIZ optional data, defensive
