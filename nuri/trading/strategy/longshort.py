...
def print_strategy(actions: list[StrategyAction]) -> None:
    ...
    print(
        f"  Target: Long {alloc.get('long_pct', 0)}% / Short {alloc.get('short_pct', 0)}% / Cash {alloc.get('cash_pct', 0)}%"
    )
    print(f"{'=' * 75}")
