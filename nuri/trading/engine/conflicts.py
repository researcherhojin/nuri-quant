    ...
    for ticker, sigs in ticker_signals.items():
        ...
        if buy_list and sell_list:
            ...
            if fit_buys and fit_sells:
                severity = "high"
                detail = (
                    f"BUY({', '.join(buy_names)})와 SELL({', '.join(sell_names)})이 모두 레짐 적합. 방향 판단 불가."
                )
                recommendation = "관망 권장. 추가 정보(펀더멘탈, 뉴스) 확인 후 판단."
            else:
                severity = "medium"
                detail = f"BUY({', '.join(buy_names)})와 SELL({', '.join(sell_names)}) 공존. 레짐 적합 시그널 우선."
            ...
            if max_pf > 3 * min_pf and min_pf < 1.5:
                ...
                conflicts.append(
                    SignalConflict(
                        ...
                        detail=(
                            f"강한 시그널({strong.signal_id}, PF={strong.profit_factor:.1f})과 "
