...
gui LaunchAgent 는 어떤 것도(watchdog 포함) 로그인 세션과 함께 죽는다. 그래서
**탐지를 기계 밖으로** 뺀다:
...
## ⚠️ 정정 — "FileVault 라서 LaunchDaemon 이관은 무효" 는 **틀렸다** (#1443)

이 독스트링은 오래 그렇게 적고 있었고, 그 문장이 올바른 대책을 막고 있었다. 2026-09-06
장애가 반증한다: `fdesetup status` 는 여전히 On 인데 `last reboot` 이 15:33 multi-user
부팅을 기록했고, 콘솔 로그인 없이 6 시간 21 분 running 이었다. FileVault 가 막는 것은
**전원 꺼진 상태의 무인 부팅**(pre-boot 물리 해제)이지 부팅 이후 시스템 도메인 데몬의
실행이 아니다. 그 창에 LaunchDaemon 이었다면 정상 동작했다.

## dead-man 은 탐지에는 옳고 **분류에는 정보가 없다** (#1443)

모든 사망이 "침묵" 하나로 수렴하는 것이 이 패턴의 장점이자 한계다. 2026-09-06 에
"머신 꺼짐" 과 "머신은 켜졌는데 에이전트 미로드" 를 구분하지 못해 운영자가 의도적
종료로 오인했고, 조치가 정반대인 두 상태라 6 시간을 방치했다. 그래서 머신 생존 신호를
**따로** 둔다 — `scripts/ops/machine_alive.sh` 가 시스템 도메인 LaunchDaemon 으로
`refs/nuri/machine-mini` 를 갱신하고, 감시자가 두 나이를 조합해 상태를 분류한다.
이 모듈의 비트는 여전히 "스케줄러가 살아 있다" 만 말한다 — 반경을 넓히지 않는다.
