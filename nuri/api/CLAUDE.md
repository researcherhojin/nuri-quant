...
- **`GET /api/scan` fetches live from yfinance on every request** — `scan_market()` → `_fetch_prices()` → `yf.download(85 tickers, 60d)`, with a populated `prices` table sitting right there. That is strategy computation plus a third-party network round trip inside a read handler, against this file's own opening rule. Since #1119 the route holds a 5-min TTL cache + single-flight (keyed by market/top — a keyless cache would serve us results to kr), so the fetch runs at most once per TTL; the structural fix (scanner reading the populated `prices` table) is still open. **Test:** `tests/api/test_swing.py::TestScanCacheSingleFlight` — nullcontext mutation fails it.
