...
- **Heavy routes pass through a slot gate — new heavy routes must join it.** 14 heavy endpoints carry `dependencies=[Depends(heavy_slot)]` (`nuri/api/limits.py`, #1119): non-blocking BoundedSemaphore(8, env `NURI_API_HEAVY_SLOTS`) — no slot → instant 503 + `Retry-After: 5`, never a queued wait (waiting itself holds a pool thread). Measured 2026-08-26: 30× concurrent `/api/rebalance` → exactly 8 ran, 22 shed, `/api/health` stayed at 0.004-0.048s (was 46.7s), zero post-load backlog. Adding a slow (≥ ~1.5s uncached) endpoint without the gate re-opens the saturation path. **Test:** `tests/api/test_limits.py::TestWiring::test_every_heavy_route_carries_the_slot_dependency` (add the path to `HEAVY_PATHS`) + `TestRouteBehavior::test_full_slots_shed_503_while_health_stays_up`.
