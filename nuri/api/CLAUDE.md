...
- **`GET /api/scan` fetches live from yfinance on every request** — `scan_market()` → `_fetch_prices()` → `yf.download(85 tickers, 60d)`, with a populated `prices` table sitting right there. That is strategy computation plus a third-party network round trip inside a read handler, against this file's own opening rule. Not yet fixed (#1119); do not copy the pattern.
