...
여기서 한 번 시도하고 실패를 기억한다. 테스트가 `sys.modules["openbb"]` 에 스텁을 꽂으면 그 스텁을
그대로 쓴다 — 스텁이 있는 동안은 실패 기억을 적용하지 않는다(실제 실패는 sys.modules 에
`openbb` 를 남기지 않으므로 둘은 구분된다).
...
def _stubbed() -> bool:
    """테스트가 sys.modules 에 꽂은 스텁이 있나 — 실제 실패는 여기 아무것도 남기지 않는다."""
    return sys.modules.get("openbb") is not None
