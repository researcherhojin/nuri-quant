...
def _get_info_panel(ticker: str) -> dict:
    ...
    # 센티먼트
    sent = query("SELECT AVG(sentiment) as avg_s, COUNT(*) as cnt FROM news WHERE ticker = ? AND sentiment IS NOT NULL", (ticker,))
    if sent and sent[0]["cnt"]:
        info["sentiment"] = sent[0]["avg_s"]
        info["news_count"] = sent[0]["cnt"]

    # 슈퍼투자자
    si = query("SELECT investor, portfolio_pct FROM superinvestors WHERE ticker = ? ORDER BY portfolio_pct DESC", (ticker,))
    if si:
...
def generate_plotly_chart(ticker: str, df: pd.DataFrame, output_dir: Path) -> Path:
    ...
    if "bb_upper" in df.columns:
        ...
        if not bb.empty:
            fig.add_trace(go.Scatter(
                x=bb.index, y=bb["bb_upper"], name="BB Upper",
                line=dict(width=0), showlegend=False,
            ), row=1, col=1)
            fig.add_trace(go.Scatter(
                x=bb.index, y=bb["bb_lower"], name="BB",
                line=dict(width=0), fill="tonexty",
                fillcolor="rgba(100,181,246,0.12)", showlegend=True,
            ), row=1, col=1)
    ...
    if info.get("target_mean"):
        fig.add_hline(
            y=info["target_mean"], line_dash="dot",
    ...
    if info.get("target_low"):
        fig.add_hline(
            y=info["target_low"], line_dash="dot",
            line_color="#ff5252", opacity=0.3,
            row=1, col=1,
        )
    ...
    if not sig_df.empty:
        ...
        if not buys.empty:
            fig.add_trace(go.Scatter(
                ...
                color="#00e676", line=dict(width=1, color="white")),
                text=buys["reason"], hovertemplate="%{text}<br>$%{y:,.2f}<extra></extra>",
            ), row=1, col=1)
        ...
        if not sells.empty:
            fig.add_trace(go.Scatter(
                ...
                color="#ff1744", line=dict(width=1, color="white")),
                text=sells["reason"], hovertemplate="%{text}<br>$%{y:,.2f}<extra></extra>",
            ), row=1, col=1)

    # ── 2. 거래량 ──
    vol_colors = ["rgba(38,166,154,0.7)" if c >= o else "rgba(239,83,80,0.7)"
    ...
    fig.add_trace(go.Scatter(
        ...
        line=dict(color="#ffab40", width=1), showlegend=False,
    ), row=2, col=1)
    ...
    if not rsi.empty:
        ...
        fig.add_hline(y=50, line_dash="dot", line_color="#666",
    ...
    if not macd_data.empty:
        ...
        fig.add_trace(go.Bar(
            x=macd_data.index, y=hist_pos, name="MACD+",
            marker_color="rgba(38,166,154,0.5)", showlegend=False,
        ), row=4, col=1)
        fig.add_trace(go.Bar(
            x=macd_data.index, y=hist_neg, name="MACD-",
            marker_color="rgba(239,83,80,0.5)", showlegend=False,
        ), row=4, col=1)

        fig.add_trace(go.Scatter(
            x=macd_data.index, y=macd_data["macd"], name="MACD",
            line=dict(color="#42a5f5", width=1.5),
        ), row=4, col=1)
        fig.add_trace(go.Scatter(
            x=macd_data.index, y=macd_data["macd_signal"], name="Signal",
    ...
    if info.get("recommendation"):
        rec_colors = {"strong_buy": "#00e676", "buy": "#66bb6a", "hold": "#ffab40",
                      "sell": "#ef5350", "strong_sell": "#d50000"}
    ...
    fig.add_annotation(
        ...
        align="right", bgcolor="rgba(30,30,30,0.85)",
        bordercolor="#444", borderwidth=1, borderpad=8,
    ...
    if not sig_df.empty:
        ...
        fig.add_annotation(
            text="<br>".join(alert_lines), xref="paper", yref="paper",
            x=0.0, y=1.0, showarrow=False,
            font=dict(size=10, color="#ccc"),
            align="left", bgcolor="rgba(30,30,30,0.85)",
    ...
    fig.update_layout(
        ...
        xaxis4=dict(
            rangeselector=dict(
                ...
                bgcolor="#1a1a2e", activecolor="#0a3d62",
...
def generate_png_chart(ticker: str, df: pd.DataFrame, output_dir: Path) -> Path:
    ...
    mpf.plot(
        ohlcv, type="candle", style="nightclouds",
        title=f"\n{ticker}", volume=True,
        addplot=addplots if addplots else None,
        figsize=(14, 10), savefig=str(path),
    )
    return path
