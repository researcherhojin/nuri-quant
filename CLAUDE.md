...
- **Sell-path threshold 는 holding row 의 account 로 resolve, `<` operator 통일** (Phase 2 A-3, PR #374, 2026-04-18): `risk_agent.py` 와 `actions.py` 는 과거 각각 `STOCK_STOP_LOSS` (-7 global) / `pnl_pct < -7` (하드코딩) 을 썼음 → long_term(-20) / pension(-30) 계좌 보유 ticker 도 -7% 손실에서 "손절선 돌파" / "urgent" 로 잘못 분류. `certification.py` 만 per-account strategy 를 올바르게 참조 → **3곳 threshold drift**. Fix: `nuri/core/rules.py::get_stop_loss_for_account(account)` — account 명 → `get_account_strategy(account)["stop_loss"]` 반환 (None/미매칭 → `STOCK_STOP_LOSS` fallback). Callers 는 **PnL 이 계산된 holding row 와 같은 account** 를 넘김 (aggregation mismatch 금지 — pnl 은 row 의 cost basis 이므로 threshold 도 row 의 account 에서 와야 함). Operator 도 `<` 로 통일 (certification.py:308 과 일치; 이전 risk_agent 는 `<=`). 새 sell-path consumer 를 작성할 때 global 상수 대신 반드시 account-scoped helper 사용 (§2.2 mechanical execution). **Test:** `tests/core/test_rules.py::TestGetStopLossForAccount` (7 cases — None/빈 account/core/long_term/pension/unknown/int type) + `tests/trading/agents/test_risk.py::TestRiskAgentA3PerAccountThreshold` (long_term -10% no-fire, long_term -22% fire, core -8% fire, boundary equality no-fire) + `tests/api/test_actions.py::TestBuildActionsLogic` (long_term -10% not-urgent, long_term -22% urgent, core -10% urgent, boundary equality not-urgent).
- **Non-emergency SELL 은 catalyst 없으면 advisory 로 downgrade** (Phase 2 A-4, PR #377, 2026-04-18): signal-기반 / consensus SELL 이 "왜 지금?" 맥락 없이 사용자에게 올라가면 위험. 이전 동작: `conflicts.py:88` 이 "추가 정보 확인" 을 **suggest** 만 하고 enforce 안 함 → actions.py 가 pnl=-2% 같은 얕은 손실 + SELL verdict 를 그대로 `check` 로 surface. Fix: `nuri/core/catalyst.py::has_recent_catalyst(ticker, ref_date, db_path)` — (a) news 14d 이내 해당 ticker, (b) macro_events 7d 이내 유의미 (confidence≥0.5, |sentiment|≥0.3) 중 하나라도 있으면 True. `candidates.py` 는 SELL + actionable 이 catalyst 없으면 `TIER_ADVISORY` 로 downgrade + `scoring_detail.catalyst_note`. `actions.py _build_actions` 는 non-breach SELL 이 catalyst 없으면 `hold` bucket 으로 강등. **stop-loss breach 는 예외** — §2.2 mechanical, actions.py `pnl < stop_loss_threshold` path 는 catalyst 함수 호출조차 하지 않고 urgent 발동. §2.6 Escalation Ladder **Soft penalty** 단계 (hard veto 아님, advisory/hold downgrade). **Test:** `tests/core/test_catalyst.py::TestHasRecentCatalyst` (9 cases — no-data, recent news, stale news, significant macro, low-conf macro, neutral-sentiment macro, stale macro, other-ticker news, news-priority-over-macro) + `tests/api/test_actions.py::TestBuildActionsLogic::test_a4_sell_no_breach_no_catalyst_becomes_hold` / `test_a4_sell_no_breach_with_catalyst_goes_to_check` / `test_a4_stop_loss_breach_bypasses_catalyst_check` (mock `assert_not_called` 로 breach path 가 catalyst 호출 안 하는지 lock).
- **Stored price 는 T-1, 장중 live divergence 는 flag 로 surface** (Phase 2 A-5, PR #379, 2026-04-18): `prices` 테이블은 일봉 close 만 저장하므로 장중 recommendation 의 `current_price` / `pnl_pct` / stop-loss 비교가 stored T-1 값 기준 → live 와 >3% 벌어져도 감지 불가. NFLX 사례 (stored $97.23 vs live $107.79, +10.87%) 가 trigger. Fix: `nuri/core/live_price.py` — `fetch_live_price(ticker)` yfinance `Ticker.fast_info.last_price` 로 live fetch (시장외/weekend/예외 시 None fallback), `check_divergence(ticker, stored, threshold=3.0)` 반환 `(is_divergent, pct, live)`. `actions.py _build_actions` 는 per-item `live_price` / `divergence_pct` / `divergence_flag` 필드 + `⚠ 실시간 시세 divergence …` reason 추가. **threshold 비교는 여전히 stored 사용 (flag 만)** — live 를 pnl/stop-loss 판정에 반영하는 것은 A-5b 로 분리 (oracle 신뢰도 관찰 후 승격, 한 번에 semantic 바꾸지 않음 — §2.6 Surface 단계). 시장외 시 `fetch_live_price` 는 rate-limit 보존 위해 skip → None. **Test:** `tests/core/test_live_price.py::TestMarketHours` (9) + `TestFetchLivePrice` (4) + `TestCheckDivergence` (6) + `tests/api/test_actions.py::TestBuildActionsLogic::test_a5_divergence_above_threshold_adds_reason` / `test_a5_divergence_below_threshold_no_reason` / `test_a5_market_closed_sets_none_fields`.
- **Multi-account ticker 는 aggregate position + worst-pnl row 로 collapse** (Phase 2 A-6, PR #381, 2026-04-18): `_get_portfolio_map` 이 동일 ticker 의 여러 계좌 보유를 largest-position row 만 keep → 다른 계좌의 stop-loss breach / divergence 가 silently masked (A-4 codex Round 1-3 연속 flag). Fix: 모든 row 를 aggregate — `position_pct` 는 합산, `pnl_pct` / `current_price` / `account` 는 worst-pnl row 기준 (stop-loss threshold 비교가 conservative 로 작동, certification.py 와 semantic 일치). 부수적으로 `getattr(c, "tier", …)` 4 call sites 제거 — `Candidate.tier` 는 dataclass default 덕분에 항상 존재, defensive lookup 은 테스트 fixture 가 real shape 과 다를 때 잠재 버그 은폐 (MockCandidate 5곳에 `tier` 필드 추가). **Test:** `tests/api/test_actions.py::TestGetPortfolioMapAggregation` (4 — single row / position 합산 / worst-pnl win / row order independence).
- **Concentration-only violation 은 SELL 경로에 흐르지 않아야 함** (PR A #429, 2026-04-21): SIEGE position_limit / concentration > MAX_SINGLE_POSITION 은 `portfolio_action=REBALANCE` 만 emit 하며, `alpha_action` 은 None, legacy `action` 은 HOLD. Consensus risk veto 는 `risk.alpha_action=="FLAT"` (or legacy fallback `alpha_action is None and action=="SELL"`) 만 발동 — concentration 단독으로는 SELL/veto 발생 불가. `/api/actions` 는 concentration violations 를 urgent 대신 새 `portfolio` bucket 으로 route ("리밸런스 권고" 언어). Stop-loss breach 는 계속 `alpha_action=FLAT` + urgent (§2.2 mechanical). **Why**: 2026-04-20 사용자 -₩7M 손실 — BAC/TSLA SIEGE REJECT 을 SELL urgent 로 surface → 매도 권장 → 다음날 급등. Codex bubble-bear Q6: *"conflates expected-return-negative with portfolio-rule-violated"*. STRATEGY §2.6 Soft-penalty (not Hard veto). **Test:** `tests/trading/agents/test_risk_alpha_portfolio_split.py::TestConcentrationOnlyDoesNotTriggerSellVeto::test_concentration_alone_emits_portfolio_action_not_sell` + `TestConcentrationHighConfidenceCannotVeto::test_consensus_does_not_veto_on_concentration_only` + `tests/api/test_actions.py::TestPRABucketRouting::test_concentration_violation_goes_to_portfolio_bucket` + `::test_stop_loss_breach_still_urgent` (concentration 에 `score -= 1` 재도입 or veto 를 `action=="SELL"` 로 되돌리면 fail).
- **SIEGE audit snapshot invariant** (E4-0a, PR #TBD, 2026-04-20): `certify()` 가 실행 중 모든 portfolio consumer 에게 동일 상태를 공급해야 audit row (regime/portfolio_hash) 와 실제 gate verdict 가 일치. 이전에는 3 곳의 `analyze_portfolio()` 직접 호출 + `_check_leverage_ban` / `_group_holdings_by_asset_class` 의 raw SQL + 별도 `_compute_portfolio_hash` DB read → 5 경로 race. Fix: `CertSnapshot` dataclass + `ContextVar[_CERT_SNAPSHOT]` 로 gate signature 변경 없이 전파. `_capture_snapshot` 이 1회 DB read (`_read_portfolio_raw`) → `_compute_portfolio_hash(rows=...)` 로 같은 rows 에서 파생 (single source). `_snapshot_portfolio_raw` / `_snapshot_portfolio` 헬퍼가 ContextVar 우선 + fresh fallback (test back-compat). `analyze_portfolio()` 실패 시 `portfolio_error` 에 저장, `_snapshot_portfolio` 가 re-raise → 각 gate 의 기존 try/except semantic 보존 (position_limit error-fail, sector/stop_loss warning-skip — empty DF 로 silent PASS 되던 regression 차단). Engine default `swallow_persist_errors=False` (loud), API routes 만 opt-in (schema drift 가 audit layer 를 조용히 무력화하는 것 차단). Hash 는 (account, ticker, sector, quantity, avg_price) — sector 포함으로 asset-class 재분류도 fingerprint. **Test:** `tests/trading/engine/test_certification_persist.py::TestSnapshotInvariant` (5) + `TestAnalyzePortfolioFailurePreservesGateSemantics` (3) + `TestRawPortfolioSnapshot` (3) + `TestCertifyPersistFailureLoudDefault` (2) + `TestComputePortfolioHash::test_hash_changes_on_sector_only_mutation`.
